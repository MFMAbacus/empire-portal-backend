...
import { registerClientSessionService } from "@/services/register-client-session-service";

// eslint-disable-next-line new-cap
export const notificationsRouter = express.Router();

notificationsRouter.post("/register", async (request, response, next) => {
  try {
    const input = combineRequestInput(request);
    const result = await registerClientSessionService.execute(input);
...
notificationsRouter.get("/getAuth", async (request, response, next) => {
  try {
